...
<div class="flex-between-start" style="background-color: #5D5FEF; height: 140px;">
    <div class="flex-c-center-start h-full">
      <div class="m-x-20">
          <span style="font-weight: bold; font-size: 20px; color: #FFFFFF;">商品状态枚举</span>
      </div>
...
function submitForm() {
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      let res = await proxy.$api.product_status_enum['update'](form.value);
...
async function getDetail(id) {
    let res = await proxy.$api.product_status_enum['get'](id);
    if(res.code == 0){
...
function refresh(){
    if(routerQuery.productStatusEnumId){
        getDetail(routerQuery.productStatusEnumId);
    }
    if(detail.value == 'all'){
        if(routerQuery.productStatusEnumId){
            getDetailAll(routerQuery.productStatusEnumId);
        }
    }
}
