...
firstSearchComment="权限名称"
:searchData="listQuery"
@refreshTableData="refreshTableData"
@addEvent="handleAdd"
uploadExcelAPI="permission.import"
table_name="permission"
firstSearchData="permissionName"
>
<template #collapse>
    <el-form-item  label="权限名称" prop="permissionName">
        <el-input clearable placeholder="请输入权限名称" v-model="listQuery.permissionName" />
    </el-form-item>

    <el-form-item  label="权限描述" prop="permissionDesc">
        <el-input clearable placeholder="请输入权限描述" v-model="listQuery.permissionDesc" />
    </el-form-item>

    <el-form-item  label="功能标识" prop="featureCode">
        <el-input clearable placeholder="请输入功能标识" v-model="listQuery.featureCode" />
    </el-form-item>
...
:query="{permissionId: scope.row.permissionId}"
text="详情"
>
</base-info-btn>
<base-edit-btn
...
async function handleDelete(row) {
  let res = await proxy.$api.permission.delete(row.permissionId);
  refreshTableData();
  proxy.$modal.msgSuccess(res.message);
}
async function handleExport() {
  try {
    await proxy.$api.permission.downloadExcel({
      ...listQuery.value
...
function submitForm() {
  if(dialogStatus.value == 'detail'){
      ...
      proxy.$refs.dataFormRef.validate(async (valid) => {
        if (valid) {
          let res = await proxy.$api.permission[dialogStatus.value](form.value);
          proxy.$modal.msgSuccess(res.message);
          refreshTableData();
          dialogVisible.value = false;
        }
