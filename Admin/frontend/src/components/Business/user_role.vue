...
@addEvent="handleAdd"
uploadExcelAPI="user_role.import"
table_name="user_role"
firstSearchData
>
<template #collapse>
    <el-form-item  label="用户ID" prop="userInfoId">
        <el-input clearable placeholder="请输入用户ID" v-model="listQuery.userInfoId" />
    </el-form-item>
...
:query="{userRoleId: scope.row.userRoleId}"
text="详情"
>
</base-info-btn>
<base-edit-btn
...
async function handleDelete(row) {
  let res = await proxy.$api.user_role.delete(row.userRoleId);
  refreshTableData();
  proxy.$modal.msgSuccess(res.message);
}
async function handleExport() {
  try {
    await proxy.$api.user_role.downloadExcel({
      ...listQuery.value
...
function submitForm() {
  if(dialogStatus.value == 'detail'){
      ...
      proxy.$refs.dataFormRef.validate(async (valid) => {
        if (valid) {
          let res = await proxy.$api.user_role[dialogStatus.value](form.value);
          proxy.$modal.msgSuccess(res.message);
          refreshTableData();
          dialogVisible.value = false;
        }
