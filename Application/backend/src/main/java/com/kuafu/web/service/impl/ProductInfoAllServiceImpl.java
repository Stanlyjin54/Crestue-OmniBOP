...
import com.kuafu.web.mapper.ProductInfoAllMapper;
 ...
 */
@Slf4j
@Service("ProductInfoAll")
public class ProductInfoAllServiceImpl extends ServiceImpl<ProductInfoAllMapper, ProductInfoAll> implements IProductInfoAllService {
@Autowired
private ProductInfoAllMapper taskInfoAllMapper;


@Override
public List<ProductInfoAll> selectListNew(PageRequest pageRequest, LambdaQueryWrapper<ProductInfoAll> queryWrapper) {

        LambdaQueryWrapper<ProductInfoAll> pageQueryWrapper = getPageQueryWrapper(pageRequest, queryWrapper);
  ...
  @Override
public List<ProductInfoAll> selectListNew(PageRequest pageRequest, LambdaQueryWrapper<ProductInfoAll> queryWrapper,boolean isPage) {
...
@Override
public long selectCount(PageRequest pageRequest, LambdaQueryWrapper<ProductInfoAll> queryWrapper) {
        LambdaQueryWrapper<ProductInfoAll> pageQueryWrapper = getPageQueryWrapper(pageRequest, queryWrapper);
...
@Override
public IPage pageNew(IPage<ProductInfoAll> page, PageRequest pageRequest, LambdaQueryWrapper<ProductInfoAll> queryWrapper) {
